    ...
    const content = (
      ...
      {sortedIngredients.map((ingredientGroup, index) => {
        const Title = () => {
          ...
          if(('calculator' in ingredientGroup && ingredientGroup.calculator) && stepNumber === 0)
            return (<RecipeCalculator calculator={ingredientGroup.calculator} quantityState={quantityState} updateQuantity={updateQuantity} ingredientGroupName={ingredientGroup.text || ""} />);
          }
          if('text' in ingredientGroup && ingredientGroup.text !== '')
            return (<h3 className="text-xl text-slate-500 mb-1"><em>For the {ingredientGroup.text}</em>{CalculatorIcon()}</h3>)
        }
        return (
          <div key={index} className={'text' in ingredientGroup && ingredientGroup.text !== '' ? 'my-4' : ''} >
          <Title/>
            {ingredientGroup.ingredients.map((ingredient, key) => {
                if (
                  (!('cooked' in ingredient) || 
                  (('cooked' in ingredient) && stepNumber >= ingredientGroup.step)) &&
